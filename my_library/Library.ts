...
const $x: any = (function (el) {
    const $ = (targets: any): any => {
        ...
        targetThis.scrollToTop = (scrollSetting:{ scrollTop:number,duration:number } = { scrollTop:0,duration:0 }) => { // 更新方法 2021/10/26
            ...
            const animationSettings:Function = (animationSetting:{
                currentTime:number,
                startVal:number,
                changeVal:number,
                animateDuration:number
            } = { currentTime:0, startVal:0, changeVal:0, animateDuration:0 }):number => {
              let { currentTime,startVal,changeVal,animateDuration }:{ currentTime:number,startVal:number,changeVal:number,animateDuration:number } = animationSetting
...
Date.prototype.getFullDateTime = function(format:{ formatType:string } | undefined): string { // 更新方法內容與回傳內容 2021/9/22
